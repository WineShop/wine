...
class StatisticsController extends AdminController {
     ...
     */
     ...
     $turn_data =$turnover->where("status='1'")->field('create_time,id')->order("id desc")->limit(1)->find();
     /*格式时间戳为 20141024*/
	 $r=date('Ymd',$turn_data['create_time']);/*格式时间戳为 20141024*/
	 if($q==$r){
		 /*当天已统计过数据，保存，创建时间不变，更新时间变化*/
         $data["update_time"]=NOW_TIME;
         $turnover->where("id={$turn_data['id']}")->save($data);
	 }else{
        /*未统计过数据，新增*/
 ...
 public function month(){  
    ...
    */
    ...
    $data["info"]=$t;
    $turn_data = $turnover->where("status='3'")->field("id,create_time")->order("id desc")->limit(1)->find();
    /*格式时间戳为 20141024*/
    $r=date('Ym',$turn_data['create_time']);/*格式时间戳为 20141024*/
